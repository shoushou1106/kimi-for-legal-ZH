...
本领域用户的实务画像保存在工作区的 legal-profile/ 目录：
  ...
  legal-profile/ai-governance-legal.md

规则：
1. 本领域所有技能从上述路径读取画像，而不是从本文件。
2. 如果该文件不存在或仍包含 [PLACEHOLDER] 标记，在进行实质工作前停下，
   引导用户先运行冷启动访谈（加载 ai-governance-legal/skills/cold-start-interview/SKILL.md）。
...
`[PLACEHOLDER]`，运行 `「cold-start-interview」工作流（加载 ai-governance-legal/skills/cold-start-interview/SKILL.md）`。*
...
*重新检查：`「cold-start-interview」工作流（加载 ai-governance-legal/skills/cold-start-interview/SKILL.md） --check-integrations`*
...
**清单文件：** `legal-profile/ai-governance-legal/ai-systems.yaml`
...
管理清单使用 `「ai-inventory」工作流（加载 ai-governance-legal/skills/ai-inventory/SKILL.md）` —— `list | add | edit <id> | classify <id> | show <id>`。
...
**验证日志。** 记录到 `legal-profile/ai-governance-legal/verification-log.md`。
...
*重新运行：`「cold-start-interview」工作流（加载 ai-governance-legal/skills/cold-start-interview/SKILL.md） --redo`*
