...
1. **直接记住**画像要点摘要——使用你（KIMI）自带的记忆功能即可。**不要通过命令行、终端或文件写入来操作记忆**（那样会陷入失败循环）。每条记忆必须以「kimi-for-legal-ZH 法律画像」开头，标注来源与适用范围，例如：「kimi-for-legal-ZH 法律画像（corporate-legal）：用户为企业法务，采购方立场，风险偏好中等……仅在处理法律工作任务时适用」。摘要内容包括：执业场景、使用者角色、所在领域、风险偏好、升级阈值，以及画像文件位置 `legal-profile/corporate-legal.md`。
