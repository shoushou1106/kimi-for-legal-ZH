...
5. 读取/写入 `legal-profile/corporate-legal/deals/[代码]/integration-tracker.yaml`。
...
存放于 `legal-profile/corporate-legal/deals/[代码]/integration-tracker.yaml`。读取 `deal-context.md` 获取交易代码、目标公司名称、交割日期和交易负责人。从 `closing-checklist.yaml`（如存在）继承任何交割后项目。
...
```
「integration-management」工作流（加载 corporate-legal/skills/integration-management/SKILL.md） --init [--deal [代码]]
```

### 第1步：加载交易上下文

读取 `legal-profile/corporate-legal/deals/[代码]/deal-context.md`。如未找到：询问交易代号、目标公司、交割日期、交易负责人和外部律师。如 deal-context.md 不存在，写入。

读取 `legal-profile/corporate-legal/deals/[代码]/closing-checklist.yaml`（如存在）。任何标记为交割后的项目成为 Day 1 或 Day 30 工作计划项（从交割检查表继承状态）。
...
> - 交割检查表——如由并购冷启动技能生成，我将自动从 `legal-profile/corporate-legal/deals/[代码]/closing-checklist.yaml` 继承
...
下一步：运行 「integration-management」工作流（加载 corporate-legal/skills/integration-management/SKILL.md） --contracts 导入
...
```
「integration-management」工作流（加载 corporate-legal/skills/integration-management/SKILL.md） --contracts [--deal [代码]]
```
...
```
「integration-management」工作流（加载 corporate-legal/skills/integration-management/SKILL.md） --report [--deal [代码]]
```
...
```
「integration-management」工作流（加载 corporate-legal/skills/integration-management/SKILL.md） --update [--deal [代码]]
```
...
```
「integration-management」工作流（加载 corporate-legal/skills/integration-management/SKILL.md） --export [--format csv|table] [--section all|consents|contracts|workplan]
```
