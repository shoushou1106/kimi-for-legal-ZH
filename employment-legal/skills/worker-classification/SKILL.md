...
1. 加载 `legal-profile/employment-legal.md` → 管辖范围、升级表。
2. 运行以下完整工作流。
...
```
「worker-classification」工作流（加载 employment-legal/skills/worker-classification/SKILL.md）
...
读取 `legal-profile/employment-legal.md` → 管辖范围、任何既有的认定历史或先前争议记录、升级表，以及团队记录的任何内部用工政策。
...
如果 `legal-profile/employment-legal.md` 记录了公司内部认定政策，先适用该政策，并标识与研究标准之间的任何冲突。
...
>
> **来源标注。** 为每个引用——每个认定标准、法规条文、规范性文件或案例——标注来源：`[yuandian检索]`用于通过插件检索的引用；`[联网检索——需复核]`用于网络搜索引用；`[模型知识——需验证]`用于模型知识回忆的引用；`[用户提供]`用于律师提供的引用。标注`需验证`的引用具有较高的编造风险，应首先核验。不得删除或压缩标注。
...
> **检索插件预检。** 在输出分析前，检查本次会话是否可连接法律研究工具——元典检索 或任何其他配置的研究工具。将结果收集到审查备注中；如果步骤2中无连接器返回结果，记录在**来源：**行中。逐条来源标签保持内联。不在输出上方单独显示横幅。
>
...
[如劳动关系成立：认定确认为劳动关系——运行 `「hiring-review」工作流（加载 employment-legal/skills/hiring-review/SKILL.md）` 审查录用通知书、竞业限制条款和管辖地特定要求。]
...
**在做出"可以按劳务关系/劳动关系/派遣/外包进行"的最终建议前：** 读取 `legal-profile/employment-legal.md` 中的 `## Who's using this`。如果角色是**非律师**：
