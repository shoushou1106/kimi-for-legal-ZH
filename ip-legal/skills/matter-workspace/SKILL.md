...
- `「matter-workspace」工作流（加载 ip-legal/skills/matter-workspace/SKILL.md） new <slug>` — 创建新事项工作区，执行简短的采集面谈，写入 `matter.md`
- `「matter-workspace」工作流（加载 ip-legal/skills/matter-workspace/SKILL.md） list` — 列明事项及其状态和活跃标记
- `「matter-workspace」工作流（加载 ip-legal/skills/matter-workspace/SKILL.md） switch <slug>` — 设置活跃事项
- `「matter-workspace」工作流（加载 ip-legal/skills/matter-workspace/SKILL.md） close <slug>` — 归档事项（移动至 `legal-profile/ip-legal/matters/_archived/`，绝不删除）
- `「matter-workspace」工作流（加载 ip-legal/skills/matter-workspace/SKILL.md） none` — 解除任何活跃事项，仅在实务级工作

## 使用说明

1. 读取 `legal-profile/ip-legal.md` — 确认 `## 事项工作区` 分区已填充。如 `Enabled` 为 `✗`，告知用户："事项工作区已关闭——你配置为公司法务，仅有一个客户，插件自动以实务级上下文运行。如你实际跨多个客户工作，重新运行 `「cold-start-interview」工作流（加载 ip-legal/skills/cold-start-interview/SKILL.md） --redo` 并选择私人执业设置。否则，你不需要 `「matter-workspace」工作流（加载 ip-legal/skills/matter-workspace/SKILL.md）`。"不用报错——关闭状态是法务用户的预期状态。
2. 按以下子命令逻辑操作。
3. 按 `$ARGUMENTS` 的首个标记分派：
   - `new` → 执行采集面谈，写入 `legal-profile/ip-legal/matters/<slug>/matter.md`，种子 `history.md` 和 `notes.md`。
   - `list` → 枚举 `legal-profile/ip-legal/matters/*/matter.md`，打印表格，标记活跃事项。
   - `switch` → 更新实务级 CLAUDE.md 中的 `Active matter:` 行。
   - `close` → 移动 `legal-profile/ip-legal/matters/<slug>/` 至 `legal-profile/ip-legal/matters/_archived/<slug>/`，在 `history.md` 中记录关闭日期。
...
**默认状态为关闭。** 法务用户永远看不到这个——他们仅在实务级运行。事项工作区在冷启动时对私人执业用户开启，或通过编辑实务级 CLAUDE.md 中的 `## 事项工作区` 开启。如 `Enabled` 为 `✗`，本技能不运行；转而解释关闭状态并建议需要事项隔离的用户运行 `「cold-start-interview」工作流（加载 ip-legal/skills/cold-start-interview/SKILL.md） --redo`。
...
6. **不**自动切换至新事项。询问："要现在切换至 `<slug>` 吗？（`「matter-workspace」工作流（加载 ip-legal/skills/matter-workspace/SKILL.md） switch <slug>`）"
...
1. 确认 `matters/<slug>/matter.md` 存在。如否，提供 `「matter-workspace」工作流（加载 ip-legal/skills/matter-workspace/SKILL.md） new <slug>`。
