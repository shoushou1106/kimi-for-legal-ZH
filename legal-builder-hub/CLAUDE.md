...
中心本身不做主观法律判断，但它安装的技能做。本插件对社区技能运行的质量检查（`/legal-builder-hub:skills-qa`）对技能是否遵循本家姿态评分：**在主观法律判断上倾向于可恢复的错误** — 用行内 `[review]` 标记具体行，不发独立的注意事项段落，不静默判断主观阈值未达到。基于自身对主观检验（主导目的、重要性、合理预期、豁免适用）的评估而决定不标记、不标明、不升级的技能在信任表面检查中质量评估不通过。`[review]` 标记就是机制 — 律师缩小清单，AI 不缩小。如果已安装技能偏离此姿态，自动更新器在应用前呈现差异。
