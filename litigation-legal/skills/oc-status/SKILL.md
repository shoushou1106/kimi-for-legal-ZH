...
如需每周运行，设置定期提醒调用 `「oc-status」工作流（加载 litigation-legal/skills/oc-status/SKILL.md）`。

1. 加载 `legal-profile/litigation-legal/matters/_log.yaml`，按默认规则（或标记）过滤。
2. 加载 `legal-profile/litigation-legal.md` → 外聘律师沟通风格、签署人默认、预算姿态。
3. 按以下工作流操作。
4. 对范围内的每个案件：读取 `matter.md` + `history.md`，起草各案邮件。
5. 将 markdown 写入 `legal-profile/litigation-legal/oc-status/[YYYY-MM-DD]/[slug].md`。
6. 写入 `legal-profile/litigation-legal/oc-status/[YYYY-MM-DD]/_summary.md` —— 运行了什么、跳过了什么及其原因。
...
- `legal-profile/litigation-legal/matters/_log.yaml` —— 过滤和字段来源
- `legal-profile/litigation-legal/matters/[slug]/matter.md` —— 案件上下文（当前姿态、待解决问题）
- `legal-profile/litigation-legal/matters/[slug]/history.md` —— 近期事件，为询问什么提供信息
- `legal-profile/litigation-legal.md` → 外聘律师沟通风格、签署人姓名/邮箱、预算姿态
...
**主题：** 按事务所惯例（来自 `legal-profile/litigation-legal.md` 外聘律师沟通风格；备用格式：`[案：[案件名称]] —— 案件进展询问`）
...
4. **预算** —— [按月/季度/按需，取决于 `legal-profile/litigation-legal.md` 预算姿态]。目前相对 [matter.md 中的预算授权] 的使用情况如何？是否有需要标注的偏差？

[如重要且相关：5. 具体要求 —— 如"请在[日期]前将起诉状/答辩状最新稿发我"——从 matter.md 待解决问题中提取。]

[署名——姓名、职务、联系方式。来自 `legal-profile/litigation-legal.md` 外聘律师沟通的签署人默认设置。]
```

根据 `legal-profile/litigation-legal.md` 外聘律师沟通风格调整语气——有些事务所是"尊敬的律师"正式风格；另一些是直呼其名加要点列表。匹配。

## 输出

### Markdown 草稿

写入至：`legal-profile/litigation-legal/oc-status/[YYYY-MM-DD]/[slug].md`
...
**发件人：** [签署人姓名/邮箱，来自 `legal-profile/litigation-legal.md`]
**主题：** [主题行]
...
处理完所有案件后，写入 `legal-profile/litigation-legal/oc-status/[YYYY-MM-DD]/_summary.md`：
...
- **强制执行最低模板。** 如果事务所语气是"一句话，直呼其名，完事"，草稿尊重此风格并跳过要点结构。匹配 `legal-profile/litigation-legal.md`。
