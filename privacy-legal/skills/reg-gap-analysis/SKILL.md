...
1. 加载 `legal-profile/privacy-legal.md` → 个人信息处理规则承诺、监管覆盖范围、DSAR系统。
2. 执行以下工作流。
...
```
「reg-gap-analysis」工作流（加载 privacy-legal/skills/reg-gap-analysis/SKILL.md） "个人信息出境标准合同办法"
```

```
「reg-gap-analysis」工作流（加载 privacy-legal/skills/reg-gap-analysis/SKILL.md）
[粘贴指引 / 法规文本]
```
...
本技能将新要求与你当前的实际情况（按 `legal-profile/privacy-legal.md` → 个人信息处理规则承诺 + PIA 中记录的实际做法）进行差异对比，并产出一份带整改计划的差距清单。
...
> 本输出中的引用由 AI 模型生成，尚未与主源核对。依赖任何法规、指引或执法行动前，请对照法律检索工具（如 元典检索 或发布机关官网）核查准确性和现行效力。AI 生成的引用可能存在编造或引用错误。每条引用上的来源标签（如 `[联网检索 — 需复核]`）显示其来源；带 `验证` 的标签造假风险更高，应优先核对。
