...
## 追加步骤：保存画像到 KIMI 记忆（KIMI 版新增）

访谈完成、画像写入 `legal-profile/{domain}.md` 后：

1. **直接记住**画像要点摘要——使用你（KIMI）自带的记忆功能即可。**不要通过命令行、终端或文件写入来操作记忆**（那样会陷入失败循环）。每条记忆必须以「kimi-for-legal-ZH 法律画像」开头，标注来源与适用范围，例如：「kimi-for-legal-ZH 法律画像（{domain}）：用户为企业法务，采购方立场，风险偏好中等……仅在处理法律工作任务时适用」。摘要内容包括：执业场景、使用者角色、所在领域、风险偏好、升级阈值，以及画像文件位置 `legal-profile/{domain}.md`。
2. KIMI 的记忆在所有会话中始终生效——标注"仅在法律工作任务中适用"是为了防止法律画像渗入无关对话（日常聊天、非法律工作）。
3. 详细审查指引以画像文件为唯一真实来源；记忆中只放摘要和文件指针，避免两处不一致。
4. 如果用户使用**网页版 KIMI**（无工作区文件系统），则跳过文件写入，直接把画像内容逐条记住（同样以「kimi-for-legal-ZH 法律画像」开头标注），后续法律会话开始时主动读取这些记忆。
"""
